...
u8 My_RTC_Init(void)
{
  ...
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR, ENABLE);//使能PWR时钟
	...
	if(RTC_ReadBackupRegister(RTC_BKP_DR0)!=0x5050)		//重新配置时间
	{
    ...
    RTC_Init(&RTC_InitStructure);
		...
		RTC_Set_Time(15,50,30,RTC_H12_PM);	//设置时间
		RTC_Set_Date(18,6,12,2);		//设置日期
	 
		RTC_WriteBackupRegister(RTC_BKP_DR0,0x5050);	//标记已经初始化过了，下次不回复位重新配置时间
	} 
